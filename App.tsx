...
const App: React.FC = () => {
  ...
  const handleSaveSchedule = async (scheduleToSave: DailySchedule) => {
    ...
    if(scheduleToSave.isLocked) alert("Posicionamento Finalizado e Sincronizado!");
  };
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <main className="flex-1 overflow-auto flex flex-col">
        <header className="bg-white shadow-sm p-6 sticky top-0 z-10 flex justify-between items-center">
          ...
          {isSyncing && <div className="flex items-center gap-2 text-blue-600 text-sm font-medium"><div className="w-2 h-2 bg-blue-600 rounded-full animate-ping" /> Gravação em curso...</div>}
        </header>
        ...
        <div className="p-6 flex-1">
          {!isLoaded ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-500 animate-pulse">
              ...
              <p className="font-medium text-lg">A carregar dados da Nuvem...</p>
            </div>
          ) : (
            <>
              {activeTab === 'settings' && <Settings settings={activeRestaurant} onSaveSettings={(updated) => setAllRestaurants(prev => prev.map(r => r.restaurantId === updated.restaurantId ? updated : r))} employees={currentEmployees} setEmployees={setCurrentEmployees} />}
              ...
              {activeTab === 'schedule_history' && (
                <ScheduleHistory 
                  ...
                  onLoadSchedule={(d) => {setTargetDate(d); setActiveTab('positioning');}} 
                  onDeleteSchedule={(d) => {
                    const updated = savedSchedules.filter(s => s.date !== d);
