...
import { supabase } from './supabaseClient'; // Importação do cliente que criaste
...
const App: React.FC = () => {
  ...
  // Estados de Dados (Sincronizados com Supabase)
  ...
  // 1. CARREGAR DA NUVEM (Ao fazer Login)
  const loadDataFromCloud = async (restaurantId: string) => {
    ...
    try {
      ...
      if (data) {
        // Se encontrou dados na nuvem, carrega-os
        ...
        // Se é um restaurante novo na nuvem, usa os valores padrão
      ...
      console.error("Erro ao comunicar com Supabase:", err);
    } finally {
      setTimeout(() => setIsLoaded(true), 500);
    }
  };

  // 2. GUARDAR NA NUVEM (Sempre que algo muda)
  ...
  // Efeito: Dispara o carregamento quando o ID muda
  useEffect(() => {
  ...
  // Efeito: Auto-save com delay (Debounce) de 2 segundos
  useEffect(() => {
    if (!isLoaded) return;
        ...
        <div className="p-4 flex items-center gap-3 border-b border-slate-700 h-16">
          ...
          {sidebarOpen && (
            <div className="overflow-hidden">
              ...
              <p className="text-[10px] text-blue-400 uppercase tracking-wider font-bold">Cloud Sync Ativo</p>
            </div>
          )}
        </div>
        ...
        <div className="p-6 flex-1">
          {!isLoaded ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-500 animate-pulse">
              ...
              <p className="font-medium text-lg">A sincronizar com a nuvem...</p>
            </div>
          ) : (
            <>
