...
import { Building2, LayoutDashboard, Sliders, TrendingUp, History, Settings as SettingsIcon, LogOut, Menu, ArrowLeft, Construction, FileText, UserCircle } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'positioning' | 'staffing' | 'sales_history' | 'schedule_history' | 'settings' | 'billing_main' | 'billing_staff'>('positioning');
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center">
        <div className="bg-white p-12 rounded-2xl shadow-lg text-center max-w-lg border border-gray-200">
           ...
           <p className="text-gray-500 mb-8">O módulo de Controlo Financeiro estará disponível em breve.</p>
      ...
      <main className="flex-1 overflow-auto flex flex-col">
        <header className="bg-white shadow-sm p-6 sticky top-0 z-10 print:hidden">
          <h2 className="text-2xl font-bold text-gray-800">
            ...
            {activeTab === 'billing_main' && 'Entregas'}
            {activeTab === 'billing_staff' && 'Gestão de Staff'}
          </h2>
        </header>
        <div className="p-6 flex-1">
          {!isLoaded ? (
            ...
            <>
              {(activeTab === 'settings' || activeTab === 'billing_staff') && <Settings settings={activeRestaurant} onSaveSettings={handleUpdateSettings} employees={currentEmployees} setEmployees={setCurrentEmployees} />}
