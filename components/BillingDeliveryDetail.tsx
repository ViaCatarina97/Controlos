...
export const BillingDeliveryDetail: React.FC<BillingDeliveryDetailProps> = ({ record, employees, onSave, onBack }) => {
  ...
  const handleLoadInvoice = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const prompt = `Analise o PDF HAVI, extraia VALOR TOTAL da tabela TOTAL POR GRUPO PRODUTO e PTO VERDE da linha TOTAL. Responda apenas JSON.`;
      ...
      if (result.groups || result.pontoVerde !== undefined) {
        setLocal(prev => {
          ...
          return { ...prev, haviGroups: updatedGroups, pontoVerde: result.pontoVerde || 0 };
        });
  ...
  return (
    <div className="flex flex-col h-full space-y-4 animate-fade-in print:p-0">
      ...
      <div className="bg-white border-2 border-purple-500 rounded-lg p-6 space-y-6 shadow-xl print:border-none print:shadow-none print:p-0">
        ...
        <div className="border border-purple-500 rounded-lg overflow-hidden">
           <div className="bg-purple-50 py-1.5 px-4 flex justify-between items-center border-b border-purple-500">
              ...
              <button onClick={() => setShowAddModal(true)} className="flex items-center gap-1.5 bg-purple-600 text-white px-3 py-1 rounded text-[10px] font-black uppercase hover:bg-purple-700 transition-colors shadow-sm">
              ...
              <div className="divide-y divide-purple-50 min-h-[40px]">
                 {local.priceDifferences.map(item => (
                    <div key={item.id} className="grid grid-cols-12 gap-2 px-4 py-2 group items-center hover:bg-purple-50/10">
                       ...
                       <div className="col-span-4"><input type="text" value={item.product} onChange={(e) => handleUpdatePriceDiff(item.id, 'product', e.target.value)} className="w-full text-[11px] text-slate-900 border-none p-0 focus:ring-0 font-medium bg-transparent" /></div>
                       <div className="col-span-2"><input type="number" value={item.priceHavi || ''} onChange={(e) => handleUpdatePriceDiff(item.id, 'priceHavi', parseFloat(e.target.value) || 0)} className="w-full text-[11px] text-slate-900 text-center border-none p-0 focus:ring-0 font-bold bg-transparent" /></div>
                       <div className="col-span-2"><input type="number" value={item.priceSms || ''} onChange={(e) => handleUpdatePriceDiff(item.id, 'priceSms', parseFloat(e.target.value) || 0)} className="w-full text-[11px] text-slate-900 text-center border-none p-0 focus:ring-0 font-bold bg-transparent" /></div>
                 ...
                 {local.priceDifferences.length === 0 && <div className="py-8 text-center text-[11px] text-gray-400 italic">Nenhum registo de diferença de preço inserido.</div>}
              </div>
           </div>
        </div>
        ...
        <div className="border border-purple-500 rounded-lg overflow-hidden">
           <div className="bg-purple-50 py-1.5 px-4 flex justify-between items-center border-b border-purple-500">
              <span className="font-black text-purple-800 uppercase text-xs tracking-wider">Produto não Introduzido</span>
              <button onClick={() => setShowMissingModal(true)} className="flex items-center gap-1.5 bg-purple-600 text-white px-3 py-1 rounded text-[10px] font-black uppercase hover:bg-purple-700 transition-colors shadow-sm">
                <Plus size={12} /> Adicionar Registo
              </button>
           </div>
              ...
              <div className="divide-y divide-purple-50 min-h-[40px]">
                 {local.missingProducts.map(item => (
                    <div key={item.id} className="grid grid-cols-12 gap-2 px-4 py-2 group items-center hover:bg-purple-50/10">
                       ...
                       <div className="col-span-1 flex justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleRemoveItem(item.id, 'missing')} className="text-red-400 p-1 hover:bg-red-50 rounded"><Trash2 size={12}/></button>
                       </div>
                    </div>
                 ))}
                 {local.missingProducts.length === 0 && <div className="py-8 text-center text-[11px] text-gray-400 italic">Nenhum produto em falta registado.</div>}
              </div>
           </div>
        </div>
        ...
        <div className="border border-purple-500 rounded-lg overflow-hidden">
           ...
           <textarea value={local.comments} onChange={(e) => setLocal({...local, comments: e.target.value})} className="w-full min-h-[80px] p-4 text-xs font-medium text-slate-900 outline-none border-none resize-none placeholder:text-gray-300 bg-white" placeholder="Escreva aqui observações ou anomalias..." />
        </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 animate-scale-up">
            <h3 className="text-lg font-black text-purple-800 uppercase mb-6 flex items-center gap-2"><Calculator className="text-purple-600" /> Diferença de Preço</h3>
            <div className="space-y-4">
              <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Grupo</label>
                <select value={newEntry.category} onChange={(e) => setNewEntry({...newEntry, category: e.target.value})} className="w-full p-2.5 border border-purple-100 rounded-lg text-sm font-bold text-slate-900 bg-white focus:ring-2 focus:ring-purple-500 outline-none">
                  {BILLING_GROUPS.map(g => <option key={g} value={g}>{g}</option>)}
                </select>
              </div>
              <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Produto</label>
                <input type="text" value={newEntry.product} onChange={(e) => setNewEntry({...newEntry, product: e.target.value})} className="w-full p-2.5 border border-purple-100 rounded-lg text-sm font-bold text-slate-900 bg-white focus:ring-2 focus:ring-purple-500 outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Preço HAVI</label>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button onClick={handleAddPriceDiff} disabled={!newEntry.product.trim()} className="flex-1 bg-purple-600 text-white font-black px-4 py-2.5 rounded-lg hover:bg-purple-700 transition-all shadow-lg disabled:opacity-50">Confirmar</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showMissingModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 animate-scale-up">
            <h3 className="text-lg font-black text-purple-800 uppercase mb-6 flex items-center gap-2"><AlertCircle className="text-purple-600" /> Produto Não Introduzido</h3>
            <div className="space-y-4">
              <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Produto</label>
                <input type="text" value={newMissing.product} onChange={(e) => setNewMissing({...newMissing, product: e.target.value})} className="w-full p-2.5 border border-purple-100 rounded-lg text-sm font-bold text-slate-900 bg-white focus:ring-2 focus:ring-purple-500 outline-none" placeholder="Nome do produto" />
              </div>
              <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Grupo</label>
                <select value={newMissing.group} onChange={(e) => setNewMissing({...newMissing, group: e.target.value})} className="w-full p-2.5 border border-purple-100 rounded-lg text-sm font-bold text-slate-900 bg-white focus:ring-2 focus:ring-purple-500 outline-none">
                  {BILLING_GROUPS.map(g => <option key={g} value={g}>{g}</option>)}
                </select>
              </div>
              <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Preço HAVI</label>
                <input type="number" value={newMissing.priceHavi || ''} onChange={(e) => setNewMissing({...newMissing, priceHavi: parseFloat(e.target.value) || 0})} className="w-full p-2.5 border border-purple-100 rounded-lg text-sm font-bold text-slate-900 bg-white focus:ring-2 focus:ring-purple-500 outline-none" placeholder="0.00" />
              </div>
              <div><label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Motivo</label>
                <select value={newMissing.reason} onChange={(e) => setNewMissing({...newMissing, reason: e.target.value})} className="w-full p-2.5 border border-purple-100 rounded-lg text-sm font-bold text-slate-900 bg-white focus:ring-2 focus:ring-purple-500 outline-none">
                  {MISSING_REASONS.map(r => <option key={r} value={r}>{r}</option>)}
                </select>
              </div>
              <div className="flex gap-3 pt-4">
                <button onClick={() => setShowMissingModal(false)} className="flex-1 px-4 py-2.5 text-gray-500 font-bold text-sm hover:bg-gray-100 rounded-lg transition-colors">Cancelar</button>
                <button onClick={handleAddMissingProductConfirm} disabled={!newMissing.product.trim()} className="flex-1 bg-purple-600 text-white font-black px-4 py-2.5 rounded-lg hover:bg-purple-700 transition-all shadow-lg disabled:opacity-50">Confirmar</button>
              </div>
            </div>
          </div>
        </div>
