...
const BILLING_GROUPS = ['Comida', 'Papel', 'F. Operacionais', 'Material Adm', 'Happy Meal', 'Outros'];
...
export const BillingDeliveryDetail: React.FC<BillingDeliveryDetailProps> = ({ record, employees, onSave, onBack }) => {
  ...
  // Lógica de Importação PDF (Mantida e Funcional)
    ...
    <div className="flex flex-col h-full space-y-4 animate-fade-in print:p-0">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex justify-between items-center print:hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => onSave({...local, isFinalized: true})} className="flex items-center gap-2 bg-purple-600 text-white px-6 py-2 rounded-lg font-bold shadow-md"><CheckCircle2 size={18}/> Finalizar</button>
        </div>
      </div>

      {/* LAYOUT DE 3 COLUNAS (COMO NA FOTO) */}
      <div className="bg-white border-2 border-purple-500 rounded-lg p-6 space-y-6 shadow-xl print:border-none">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="border border-purple-500 rounded-lg overflow-hidden flex flex-col">
            ...
            <div className="p-1 flex flex-col flex-1 divide-y divide-purple-100">
               ...
               {local.haviGroups.map(group => (
                 <div key={group.group} className="grid grid-cols-12 gap-1 px-2 py-1 items-center hover:bg-purple-50/50">
          ...
          <div className="border border-purple-500 rounded-lg overflow-hidden flex flex-col">
            ...
            <div className="p-1 flex flex-col flex-1 divide-y divide-purple-100">
               ...
               <div className="mt-auto p-3 bg-purple-50 text-center border-t border-purple-500">
                  <div className="text-xl font-black text-purple-900">{totalMyStore.toFixed(2)} €</div>
                  <div className="text-[8px] font-bold text-purple-400 uppercase">TOTAL MYSTORE CONSOLIDADO</div>
               </div>
            </div>
          </div>
          ...
          <div className="border border-purple-500 rounded-lg overflow-hidden flex flex-col">
            ...
            <div className="p-1 flex flex-col flex-1 divide-y divide-purple-100">
               ...
               {local.smsValues.map(v => (
                 <div key={v.description} className="grid grid-cols-12 gap-1 px-2 py-2 items-center">
                    <div className="col-span-9 text-[10px] font-bold text-gray-700">{v.description}</div>
                    <div className="col-span-3 text-right text-[10px] font-black text-red-500">0.00 €</div>
                 </div>
               ))}
               <div className="mt-auto p-6 text-center flex flex-col items-center border-t border-purple-500">
                  <div className="text-4xl font-black text-slate-900">{finalDifference.toFixed(2)} €</div>
                  <div className="w-16 h-1 bg-red-500 my-2"></div>
                  <div className="text-[8px] font-bold text-gray-400 uppercase">DIFERENÇA FINAL FINAL</div>
               </div>
            </div>
          </div>
        </div>

        {/* SECÇÃO INFERIOR: DIFERENÇAS DE PREÇO E NÃO INTRODUZIDO */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4">
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
            <div className="flex justify-between items-center mb-2 border-b pb-2">
              ...
              <Plus size={14} className="text-purple-600 cursor-pointer"/>
            </div>
            <div className="grid grid-cols-4 text-[8px] font-black text-slate-400 uppercase pb-2">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
          ...
          <textarea className="w-full h-20 bg-transparent border-none text-sm focus:ring-0" placeholder="Notas da conferência..."></textarea>
        </div>
      </div>
    </div>
  );
